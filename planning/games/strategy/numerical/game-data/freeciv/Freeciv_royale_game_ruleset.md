...
> 原始链接：https://github.com/longturn/freeciv21/blob/main/data/royale/game.ruleset
> 分类：numerical
> 标签：文明, 4X策略, 规则集, royale

## 概述
Freeciv royale规则集的game定义
...
[datafile]
description="Royale game rules for Freeciv21"
options="+Freeciv-ruleset-Devel-2017.Jan.02"
format_version=20

; This section contains meta information for freeciv21-ruledit to recreate the ruleset
; file in a form wanted. These have no in-game effect whatsoever
[ruledit]

; Which file to read description in from.
description_file = "royale/README.royale"

[about]
; Ruleset name
name = _("Royale ruleset")

; There`s no separate versioning in rulesets part of main Freeciv21 distribution
;version = ""
...
; version of Freeciv21 can find the ruleset from the correct place when loading
; the savegame to the future version.
;alt_dir = ""

; Summary of the ruleset
; /* TRANS: In the client, this is displayed alongside the contents of
;    README.royale, which are not localized. */
summary = _("\
You are playing Freeciv21 with Royale ruleset based on Longturn Traditional LTT rules (originally based on civ2civ3):\
\n\n\
Gameplay more similar to classic 4x empire building games, keeping it easy to learn but challenging to master.\
\n\n\
Also suitable for single player, with AI less aggressive at start,\
 but more challenging at the end.\
\n\n\
The Royale ruleset is designed to be played without AI players and allows very fast gameplay\
 meaning that one game does not take too much time. For similar gameplay with more turns look for the LTT ruleset.\
 and it is balanced to make the game interesting for both beginners and experienced players.\
 Everything comes with a cost. Even advancing more than your opponent. This keeps it challenging and fair to everyone.\
")

; Detailed description
; When updating this, update also desciption_file in [ruledit] section to match
description = *royale/README.royale*
...
granary_food_ini = 12, 14, 16, 20, 22, 26, 32, 30
granary_food_inc = 0
...
; here is a comment about losing the veteran levels. read it before changing this ever again. this has been tried several times.
; "losing level is wrong. makes early barracks/battle less interesting. so we make the game simpler with less strategic choices."
...
; The diplomatic state between two nations that meet for the first time
; One of "War", "Cease-fire", "Armistice", "Peace", "Alliance"
initial_diplomatic_state = "Armistice"
...
nuke_defender_survival_chance_pct = 20
...
; The maximum distance from the actor unit to the target of the
; "Recycle Unit" action. The value 0 means that the target`s tile must be the
; tile of the actor unit. The value 1 means that the city must be on a tile
; adjacent to the actor unit. The special value "unlimited" lifts the
; maximum distance restriction. The maximum distance can`t be smaller than
; the minimum distance.
recycle_unit_max_range = 1
...
; The first %s should be before the mnemonic of the action. A Freeciv21 client
...
ui_name_transport_alight = _("%sAlight%s")
...
; tile. "Conquer City 2" has the same relation to "Conquer City".
quiet_actions = "Transport Disembark 2", "Conquer City 2"
...
[actionenabler_steal_tech_random_scribe]
action = "Steal Tech"
actor_reqs    =
    { "type",   "name", "range"
      "UnitType", "Scribe", "Local"
      "UnitState", "OnLivableTile", "Local"
      "MinMoveFrags", "1", "Local"
      "DiplRel",      "Foreign",       "Local", TRUE
    }
...
target_reqs  =
    { "type",     "name",                  "range", "present"
      ...
      "Building", "2nd Palace", "City",   FALSE
...
[actionenabler_paradrop_base_to_foreign]
...
[actionenabler_paradrop_city_to_foreign]
action = "Paradrop Unit"
actor_reqs    =
    { "type",         "name",         "range", "present"
      "UnitFlag",     "Paratroopers", "Local", TRUE
...
[teams]
; Freeciv21 optional team names definition.
;
; names =
; _("Team 1"),
; _("Team 2"),
...
[settings]
; Freeciv21 game settings for royale ruleset
;
set =
    { "name", "value"
     "aifill", 6
