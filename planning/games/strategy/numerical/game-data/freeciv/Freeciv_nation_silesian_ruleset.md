# Freeciv(nation) · silesian

> 来源：longturn/freeciv21
> 原始链接：https://github.com/longturn/freeciv21/blob/main/data/nation/silesian.ruleset
> 分类：numerical
> 标签：文明, 4X策略, 规则集, nation

## 概述
Freeciv nation规则集的silesian定义

## 正文
```ruleset
[nation_silesian]

name=_("Silesian")
plural=_("?plural:Silesians")
groups="Medieval", "European"
legend=_("Silesia is a region of southern Poland that was at one time an\
 independent kingdom.")

leaders = {
 "name",                "sex"
 "Władysław Wygnaniec", "Male"
 "Henryk Brodaty",      "Male"
 "Henryk Pobożny",      "Male"
 "Jerzy IV Wilhelm",    "Male"
 "Józef Kożdoń",        "Male"
 "Jan Kustos",          "Male"
 "Wojciech Korfanty",   "Male"
}

ruler_titles = {
 "government",      "male_title",           "female_title"
 "Despotism",        _("Prince %s"),        _("Princess %s")
 "Fundamentalism",   _("Bishop %s"),        _("Mother Superior %s")
}

flag="silesia"
flag_alt = "-"
style = "European"
init_techs=""
init_buildings=""

init_units=""
civilwar_nations = "polish", "sorbian", "moravian"

; if you want your favourite city to be in more prominent place, go ahead
; only leave first 3 cities where they are

cities =
  "Wrocław",
  "Opole",
  "Katowice",
  "Legnica",
  "Zielona Góra (hills, grassland)",
  "Głogów (forest)",
  "Bolesławiec",
  "Wałbrzych",
  "Gliwice",
  "Rybnik (river)",
  "Jelenia Góra (mountains)",
  "Świdnica",
  "Lubin",
  "Chojnów",
  "Małczyce", 
  "Kłodzko",
  "Bytom",
  "Oświecim",
  "Złotoryja",
  "Bielany",
  "Zgorzelec", 
  "Żary",
  "Bystrzyca (river)",
  "Żagan",
  "Bogatynia", 
  "Chocianów",
  "Nowa Sól",
  "Zabrze",
  "Sosnowiec (forest)",
  "Rokitki",
  "Lubań",
  "Polkowice",
  "Lubiąż",
  "Kędzierzyn-Koźle",
  "Chorzów",
  "Bielsko",
  "Cieszyn",
  "Siemianowice Śląskie",
  "Wojkowice",
  "Lubliniec",
  "Będzin",
  "Czeladź"

```

## 策划参考价值
4X SLG鼻祖级数值参考：单位/建筑/科技树/文明特性。
